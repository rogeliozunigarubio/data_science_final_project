...
mypath = "MAR14/"
'''
captured = open('captured_reviews.csv', 'wt')
captured_text = open('captured_reviews_text_only.txt', 'wt')
'''
for nextFile in listdir(mypath):
	if isfile(mypath+nextFile):
		reviewCodes = nextFile.split("-",3)
		if reviewCodes[0] == "Attraction_Review":
...
#				print nextTag
				...
				print(("{0},{1},{2},{3}").format(reviewCodes[1],reviewCodes[2],reviewDate, reviewText)+"\n")
'''
